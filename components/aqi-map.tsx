...
const getAQIStatus = (aqi: number): { text: string; color: string } => {
  ...
  if (aqi <= 150)
    return { text: "Unhealthy for Sensitive Groups", color: "#ff7e00" };
...
export function AQIMap() {
  ...
  // Search handler with corrected template literals
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        map.flyTo([lat, lon], 12, { duration: 3, easeLinearity: 0.1 });

        const [aqiData, weatherResponse] = await Promise.all([
          AQIService.getNearestStation(Number(lat), Number(lon)),
          fetch(
            `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${process.env.NEXT_PUBLIC_OPENWEATHER_KEY}`
          ),
        ]);
              ...
              <div class="bg-background/50 p-3 rounded-lg border border-border/50">
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <div class="text-2xl sm:text-3xl font-bold" style="color: ${
                      status.color
                    }">${aqiData.aqi}</div>
          ...
          status.color
              ...
              <div class="grid grid-cols-2 gap-2">
                <div class="bg-background/50 p-3 rounded-lg border border-border/50">
                  ...
                  <div class="text-xl font-bold">${kelvinToCelsius(
                ...
                <div class="bg-background/50 p-3 rounded-lg border border-border/50">
                  ...
                  <div class="text-xs text-muted-foreground">Wind: ${Math.round(
        ...
        const circleColor = getAQIColor(aqiData.aqi);
        ...
        const animate = () => {
          ...
          radius += 100;
          if (opacity <= 0.2) {
            circle.setStyle({ fillOpacity: opacity, radius: radius });
    ...
    leafletMap.on("click", async (e: any) => {
      ...
      try {
        ...
        if (data) {
          ...
          const clickIcon = createCustomIcon(color);
  ...
  return (
    <div className="fixed inset-0 pt-16">
      ...
      <div className="w-full h-[calc(100vh-5rem)] pb-16">
        ...
        {/* Updated AQI Legend - Mobile Optimized */}
        <Card className="absolute bottom-20 left-4 right-4 sm:right-auto p-3 bg-background/95 backdrop-blur-sm z-[400] shadow-lg">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs map-legend-mobile">
